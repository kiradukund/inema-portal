...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const installments = calc.schedule.map((s, i) => ({
      loan_id: loan.id, client_name: clientName, num: i + 1,
    ...
    await supabase.from('loans').insert({
      ...
      status: 'active', disbursed_at: new Date().toISOString(), total_repayment: calc.total_repayment,
    })

    return ok({ message: `Approved for ${clientName}. RWF ${approved_amount.toLocaleString()} / ${approved_term}mo.`, loan_id: loan.id, client_phone: profile?.phone ?? '', total_repayment: calc.total_repayment })
