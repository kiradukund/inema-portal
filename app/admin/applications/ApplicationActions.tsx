...
interface Props { id: string; clientName: string; clientPhone: string; amount: number; term: number }
    ...
    <div className="flex flex-col gap-2 min-w-[180px]">
      {!showApprove && !showReject && (
        <div className="flex gap-2">
          <button onClick={() => setShowApprove(true)} className="text-xs bg-green-600 text-white px-3 py-1.5 rounded-lg hover:bg-green-700 font-semibold">✓ Approve</button>
          <button onClick={() => setShowReject(true)}  className="text-xs bg-red-50 text-red-600 border border-red-200 px-3 py-1.5 rounded-lg hover:bg-red-100 font-semibold">✗ Reject</button>
        </div>
      )}
      {showApprove && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-3 space-y-2">
          ...
          <div className="flex gap-2">
            <div><label className="text-xs text-green-700">Amount (RWF)</label>
              <input type="number" value={approveAmount} onChange={e => setApproveAmount(e.target.value)} className="w-28 text-xs border border-green-300 rounded px-2 py-1 block mt-0.5" /></div>
            <div><label className="text-xs text-green-700">Term (mo)</label>
              <input type="number" min={1} max={6} value={approveTerm} onChange={e => setApproveTerm(e.target.value)} className="w-14 text-xs border border-green-300 rounded px-2 py-1 block mt-0.5" /></div>
          </div>
          <div className="flex gap-2">
            <button onClick={approve} disabled={loading==='approve'} className="text-xs bg-green-600 text-white px-3 py-1.5 rounded-lg hover:bg-green-700 font-semibold disabled:opacity-60">
      ...
      {showReject && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-3 space-y-2">
          ...
          <textarea value={rejectNote} onChange={e => setRejectNote(e.target.value)} placeholder="Reason (optional)..."
            className="w-full text-xs border border-red-300 rounded px-2 py-1 min-h-[48px] resize-none" />
          <div className="flex gap-2">
            <button onClick={reject} disabled={loading==='reject'} className="text-xs bg-red-600 text-white px-3 py-1.5 rounded-lg hover:bg-red-700 font-semibold disabled:opacity-60">
