...
export default async function AdminApplications() {
  ...
  // Step 1: fetch applications WITHOUT join
  ...
  if (appsError) {
    return (
      <div className="p-8">
        <h1 className="text-2xl font-bold text-red-600 mb-2">Error loading applications</h1>
        <pre className="text-sm bg-red-50 p-4 rounded text-red-800">{JSON.stringify(appsError, null, 2)}</pre>
      </div>
    )
  }
  ...
  // Step 2: fetch profiles separately for all client_ids
  const clientIds = [...new Set(all.map(a => a.client_id).filter(Boolean))]
  let profileMap: Record<string, any> = {}
  if (clientIds.length > 0) {
    const { data: profiles } = await supabase
      .from('profiles')
      .select('id, full_name, phone, employer_name, monthly_income')
      .in('id', clientIds)
    ;(profiles ?? []).forEach(p => { profileMap[p.id] = p })
  }

  const counts = { submitted: 0, approved: 0, rejected: 0 }
  all.forEach(a => { if (a.status in counts) counts[a.status as keyof typeof counts]++ })

  const loanLabel: Record<string, string> = {
    salary_advance: 'Salary Advance', quinzaine: 'Quinzaine',
    school_fees: 'School Fees', business: 'Business'
  }
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-slate-500 text-sm mt-1">Portal applications from registered clients ({all.length} total)</p>
      </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {all.map(app => {
                  ...
                  const docsReady = [
                    app.has_id_copy, app.has_payslips, app.has_bank_statement,
                    app.has_employment_letter, app.has_application_letter
                  ...
                  return (
                    <tr key={app.id} className="border-b border-slate-50 hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 text-slate-500 max-w-[140px] truncate" title={app.purpose ?? ''}>{app.purpose ?? '—'}</td>
                      <td className="px-4 py-3 text-slate-500 whitespace-nowrap">{app.employer ?? profile.employer_name ?? '—'}</td>
                      <td className="px-4 py-3">
                        <span className={`text-xs font-semibold px-2 py-1 rounded-full ${docsReady >= 3 ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                          {docsReady}/5
                        </span>
                      </td>
                      <td className="px-4 py-3 text-slate-500 text-xs whitespace-nowrap">
                        {app.submitted_at ? new Date(app.submitted_at).toLocaleDateString('en-RW') : '—'}
                      </td>
                      <td className="px-4 py-3">
                        <span className={`text-xs font-semibold px-2 py-1 rounded-full capitalize ${statusColor[app.status] ?? 'bg-slate-100 text-slate-600'}`}>
                          {app.status}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        {app.status === 'submitted' ? (
                          ...
                          <span className="text-xs text-slate-400 max-w-[120px] block truncate">
                            {app.review_notes?.slice(0, 50) || 'Processed'}
                          </span>
                        )}
                      </td>
                    </tr>
