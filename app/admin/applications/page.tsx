...
export default async function AdminApplications() {
  ...
  const pending = all.filter(a => a.status === 'submitted' || a.status === 'under_review' || a.status === 'draft')
              ...
              <div key={app.id} className="p-5 flex items-start justify-between gap-4">
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-3">
                    ...
                    <div><p className="text-xs text-slate-400">Amount Requested</p><p className="text-sm font-semibold text-slate-700">{rwf(app.requested_amount ?? 0)}</p></div>
                  ...
                  <div className="flex gap-6 text-sm mb-2">
                    ...
                    {app.employer && <span><span className="text-slate-400">Employer: </span><span className="text-slate-600">{app.employer}</span></span>}
                  </div>
                  {app.purpose && <p className="text-sm text-slate-600 bg-slate-50 rounded p-2"><b>Purpose:</b> {app.purpose}</p>}
                  <div className="flex gap-4 mt-2 text-xs text-slate-400">
                    {app.has_id_copy && <span>✅ ID Copy</span>}
                    {app.has_payslips && <span>✅ Payslips</span>}
                    {app.has_bank_statement && <span>✅ Bank Statement</span>}
                    {app.has_employment_letter && <span>✅ Employment Letter</span>}
                  </div>
                </div>
                <div className="flex flex-col gap-2 min-w-[130px]">
                  ...
                  <a href={`https://wa.me/${(app.profiles?.phone ?? '').replace(/\D/g, '')}?text=${encodeURIComponent('Dear ' + (app.profiles?.full_name ?? '') + ', your INEMA loan application for ' + rwf(app.requested_amount ?? 0) + ' is under review. We will contact you within 24 hours. Call +250788834132.')}`}
                    target="_blank"
                    className="w-full bg-green-500 text-white text-sm font-bold px-5 py-2.5 rounded-lg text-center">
                    💬 WhatsApp
                  </a>
                </div>
              </div>
              ...
              <tbody>
                {all.map((app: any) => (
                  <tr key={app.id} className="border-b border-slate-50 hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-1 rounded-full font-medium ${sc[app.status] ?? 'bg-slate-100 text-slate-600'}`}>{app.status}</span>
                    </td>
                    <td className="px-4 py-3 text-xs text-slate-500">{new Date(app.created_at).toLocaleDateString()}</td>
                    <td className="px-4 py-3">
                      {(app.status === 'submitted' || app.status === 'under_review' || app.status === 'draft') && (
                        <div className="flex gap-2">
                          <form action={`/api/admin/applications/${app.id}/approve`} method="POST">
                            <button type="submit" className="bg-green-100 text-green-700 text-xs font-bold px-3 py-1.5 rounded-lg hover:bg-green-200">✓</button>
                          </form>
                          <form action={`/api/admin/applications/${app.id}/reject`} method="POST">
                            <button type="submit" className="bg-red-100 text-red-700 text-xs font-bold px-3 py-1.5 rounded-lg hover:bg-red-200">✕</button>
                          </form>
                        </div>
                      )}
                    </td>
