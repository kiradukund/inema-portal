    ...
    <div className="min-h-screen flex bg-slate-50">
      <aside className="w-60 bg-slate-900 flex flex-col flex-shrink-0">
        ...
        <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
          {nav.map(i=>(
            <Link key={i.href} href={i.href} className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-300 hover:bg-slate-800 hover:text-white transition-colors text-sm font-medium">
              <span>{i.icon}</span>{i.label}
            </Link>
          ))}
        </nav>
        <div className="p-3 border-t border-slate-800">
          <Link href="/dashboard" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-400 hover:bg-slate-800 hover:text-white text-sm"><span>↩</span> Client Portal</Link>
          <form action="/api/auth/logout" method="POST">
            <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-400 hover:bg-slate-800 hover:text-white text-sm"><span>🚪</span> Sign Out</button>
          </form>
        </div>
      </aside>
