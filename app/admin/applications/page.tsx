...
export default async function AdminApplications() {
  ...
  const { data: apps } = await admin.from('loan_applications').select('*, profiles(full_name, email, phone)').order('created_at', { ascending: false })
  ...
  const sc: Record<string,string> = {
    ...
    approved:'bg-green-100 text-green-700', rejected:'bg-red-100 text-red-700', draft:'bg-slate-100 text-slate-600'
  }
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-slate-500 text-sm mt-1">{pending.length} pending · {all.filter(a=>a.status==='approved').length} approved · {all.filter(a=>a.status==='rejected').length} rejected</p>
      </div>

      <div className="grid grid-cols-4 gap-4 mb-6">
        {[{l:'Total',c:all.length,b:'border-slate-400'},{l:'Pending',c:pending.length,b:'border-amber-500'},{l:'Approved',c:all.filter(a=>a.status==='approved').length,b:'border-green-500'},{l:'Rejected',c:all.filter(a=>a.status==='rejected').length,b:'border-red-500'}].map(x=>(
              ...
              <div key={app.id} className="p-5 flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-3">
                    <p className="font-bold text-slate-800 text-lg">{app.profiles?.full_name ?? 'Unknown'}</p>
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-3">
                    ...
                    <div><p className="text-xs text-slate-400">Amount</p><p className="text-sm font-semibold text-slate-700">{rwf(app.amount??0)}</p></div>
                  ...
                  <div className="flex gap-6 text-sm">
                    <span><span className="text-slate-400">Phone: </span><a href={`tel:${app.profiles?.phone??''}`} className="text-blue-600 font-medium">{app.profiles?.phone??'—'}</a></span>
                    <span><span className="text-slate-400">Email: </span><span className="text-slate-600">{app.profiles?.email??'—'}</span></span>
                  </div>
                ...
                <div className="flex flex-col gap-2 min-w-[130px]">
                  ...
                  <a href={`https://wa.me/${(app.profiles?.phone??'').replace(/\D/g,'')}?text=${encodeURIComponent('Dear '+(app.profiles?.full_name??'')+', your INEMA loan application for '+rwf(app.amount??0)+' is under review. We will contact you within 24 hours. Call +250788834132.')}`} target="_blank" className="w-full bg-green-500 text-white text-sm font-bold px-5 py-2.5 rounded-lg text-center">💬 WhatsApp</a>
                </div>
              </div>
            ))}
          </div>
              ...
              <tbody>
                {all.map((app:any)=>(
                  <tr key={app.id} className="border-b border-slate-50 hover:bg-slate-50">
                    <td className="px-4 py-3"><p className="font-semibold text-slate-800">{app.profiles?.full_name??'—'}</p><p className="text-xs text-slate-400">{app.profiles?.email??''}</p></td>
                    ...
                    <td className="px-4 py-3 font-semibold text-slate-800">{rwf(app.amount??0)}</td>
                    ...
                    <td className="px-4 py-3">
                      {(app.status==='submitted'||app.status==='under_review')&&(
                        <div className="flex gap-2">
                          <form action={`/api/admin/applications/${app.id}/approve`} method="POST"><button type="submit" className="bg-green-100 text-green-700 text-xs font-bold px-3 py-1.5 rounded-lg">✓</button></form>
                          <form action={`/api/admin/applications/${app.id}/reject`} method="POST"><button type="submit" className="bg-red-100 text-red-700 text-xs font-bold px-3 py-1.5 rounded-lg">✕</button></form>
                        </div>
                      )}
                    </td>
                  </tr>
