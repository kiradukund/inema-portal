            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  {['Ref#','Client','Phone','Type','Amount','Term','Purpose','Employer','Docs','Submitted','Status','Actions'].map(h => (
              ...
              <tbody>
                {all.map(app => {
                  ...
                  const docsReady = [app.has_id_copy, app.has_payslips, app.has_bank_statement, app.has_employment_letter, app.has_application_letter].filter(Boolean).length
                  return (
                    <tr key={app.id} className="border-b border-slate-50 hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-xs font-semibold px-2 py-1 rounded-full ${docsReady >= 3 ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>{docsReady}/5</span>
                      </td>
                      <td className="px-4 py-3 text-slate-500 text-xs whitespace-nowrap">{app.submitted_at ? new Date(app.submitted_at).toLocaleDateString('en-RW') : '—'}</td>
                      <td className="px-4 py-3">
